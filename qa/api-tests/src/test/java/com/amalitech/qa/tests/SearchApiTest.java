...
public class SearchApiTest extends SetUp {
    ...
    static void createSearchFixtures() {
        ...
        fixtureOwnerToken = registerAndGetTokenWithRetry("Search Fixture Owner", ownerEmail, "password123");
        for (String category : CATEGORIES) {
            PostRequest request = new PostRequestBuilder()
                    .withTitle("Search Test " + category)
                    .withBody("Search test body content for category " + category)
    ...
    void search_byKeyword_returns200() {
        RestAssured.given()
                .spec(requestSpec)
                .queryParam("keyword", "Search Test")
                .when()
                .get("/api/posts/search")
                .then()
                .statusCode(200)
    ...
    void search_allFilters_returns200() {
        ...
        RestAssured.given()
                ...
                .queryParam("keyword", "Search Test NEWS")
                .queryParam("startDate", startDate)
                .queryParam("endDate", endDate)
                .when()
                .get("/api/posts/search")
    ...
    void search_noMatch_returnsEmptyPage() {
        RestAssured.given()
                .spec(requestSpec)
                .queryParam("keyword", "zzznomatch999xyz")
                .when()
                .get("/api/posts/search")
                .then()
                .statusCode(200)
